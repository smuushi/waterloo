    ...
    <div className="flex bg-base-300 relative pb-10">
      <div className="flex flex-col w-full mx-5 sm:mx-8 2xl:mx-20">
        <div className="flex flex-col mt-6 px-7 py-8 bg-base-200 opacity-80 rounded-2xl shadow-lg border-2 border-primary">
          <span className="text-4xl sm:text-6xl text-black">Welcome to FounderVille</span>
          <br/>
          <div>
            {villagerTitle()}
          </div>
          <div className="flex flex-col">
            {Object.values(yourVillagers).map((villager, idx) => (
              <a href={`https://tokenbound.org/assets/polygon/${contractAddress}/${idx}`} target="_blank" className="bg-white transition-colors duration-300 ease-in-out hover:bg-black">{villager.name}</a>
            ))}
          </div>
        </div>
      </div>
